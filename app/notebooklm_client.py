...
class NotebookLMClient:
    ...
    async def upload_document(self, file_path: str | pathlib.Path) -> Dict[str, Any]:
        ...
        create_button_selector = upload_cfg.get("create_notebook_button")
        notebook_ready_selector = upload_cfg.get("notebook_ready_marker")
        notebook_timeout = self._config.timeouts.get("notebook", self._navigation_timeout)

        if create_button_selector:
            LOGGER.debug(
                "Ensuring a notebook is available by checking %s", create_button_selector
            )
            try:
                ...
                LOGGER.debug(
                    "Create notebook button not visible within %sms; assuming workspace is ready",
                    notebook_timeout,
                )
            else:
                LOGGER.info("Clicking the create notebook button (%s)", create_button_selector)
                await self._page.click(create_button_selector)
                if notebook_ready_selector:
                    try:
                        await self._page.wait_for_selector(
                            notebook_ready_selector,
                            state="visible",
                            timeout=notebook_timeout,
                        )
                    except TimeoutError:
                        LOGGER.warning(
                            "Notebook ready marker (%s) did not appear within %sms",
                            notebook_ready_selector,
                            notebook_timeout,
                        )
